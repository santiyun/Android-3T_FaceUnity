...
public class MainActivity extends BaseActivity {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // **注意：此 Demo 仅展示 相芯美颜 SDK 基本的美颜效果，验证 SDK 提供的裸数据没有问题。其他高级美颜或道具效果，
        // 请参考 FURenderer 类中的接口介绍或 相芯美颜 相关文档介绍。
        // 使用 相芯美颜 SDK 之前，需要把三体的自带美颜功能关闭，调用下面的接口。
        mTTTEngine.setBeautyFaceStatus(false, 0f, 0f);
        // 相芯美颜 SDK 全局初始化
        FURenderer.initFURenderer(this);
        LocalConfig.mFURenderer = new FURenderer.Builder(this)
                .maxFaces(2)
                .inputTextureType(2)
                .setNeedFaceBeauty(true)
                .setOnSystemErrorListener(new FURenderer.OnSystemErrorListener() {
                    @Override
                    public void onSystemError(String error) {
                        Log.i("Captured", String.format("onSystemError = %s", error));
                    }
                })
                .build();
    ...
    private void initView() {
        ...
        if (LocalConfig.mLocalRole == CLIENT_ROLE_ANCHOR) {
            ...
            // 添加 SurfaceView 控件的生命周期回调监听，用于 相芯美颜 SDK的相关操作
            mSurfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
                @Override
                public void surfaceCreated(SurfaceHolder holder) {
                    LocalConfig.mFURenderer.onSurfaceCreated();
                }
                ...
                public void surfaceDestroyed(SurfaceHolder holder) {
                    LocalConfig.mFURenderer.onSurfaceDestroyed();
                }
            });
        }
